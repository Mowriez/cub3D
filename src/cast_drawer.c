...
/*   Updated: 2023/08/05 07:23:07 by mtrautne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_pixel_color(t_vars *vrs, double wall_y)
{
	...
	{
		tex_x = (int)(vrs->tx_n->width * (vrs->ray_pos_x - floor(vrs->ray_pos_x)));
	...
	{
		tex_x = (int)(vrs->tx_s->width * (vrs->ray_pos_x - floor(vrs->ray_pos_x)));
		tex_y = (int)floor(vrs->tx_s->height * wall_y);
		char *pixel = vrs->tx_s->img_data_addr + (tex_x * vrs->tx_s->bits_p_px / 8) \
				+ (tex_y * vrs->tx_s->ln_len);
		vrs->wall_color = *(unsigned int *)pixel;
	}
	else if (vrs->wall_side == FACING_EAST)
	{
		tex_x = (int)(vrs->tx_e->width * (vrs->ray_pos_y - floor(vrs->ray_pos_y)));
		tex_y = (int)floor(vrs->tx_e->height * wall_y);
		char *pixel = vrs->tx_e->img_data_addr + (tex_x * vrs->tx_e->bits_p_px / 8) \
				+ (tex_y * vrs->tx_e->ln_len);
		vrs->wall_color = *(unsigned int *)pixel;
	}
	else if (vrs->wall_side == FACING_WEST)
	{
		tex_x = (int)(vrs->tx_w->width * (vrs->ray_pos_y - floor(vrs->ray_pos_y)));
		tex_y = (int)floor(vrs->tx_w->height * wall_y);
		char *pixel = vrs->tx_w->img_data_addr + (tex_x * vrs->tx_w->bits_p_px / 8) \
...
void	draw_vert_line(int x, t_vars *vrs)
{
	...
	while (y < y_stop)
	{
		wall_y = (y - y_start) / (double)vrs->wall_height;
		// printf("wall_y: %f\n", wall_y);
		set_pixel_color(vrs, wall_y);
