...
/*   Updated: 2023/08/05 20:01:22 by mtrautne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	move_up_or_back(t_vars *vrs)
{
	...
	if (vrs->key_state[0] == KEY_PRESSED)
	{
		...
		double new_x_test = vrs->player_pos_x + (HIT_BOX * cos(vrs->view_angle) * MV_SP);
		double new_y_test = vrs->player_pos_y + (HIT_BOX * sin(vrs->view_angle) * MV_SP);
...
static void	move_sideways(t_vars *vrs)
{
	...
	if (vrs->key_state[2] == KEY_PRESSED)
	{
		...
		double new_x_test = vrs->player_pos_x + (HIT_BOX * cos(vrs->view_angle - 0.5 * M_PI) * MV_SP);
		double new_y_test = vrs->player_pos_y + (HIT_BOX * sin(vrs->view_angle - 0.5 * M_PI) * MV_SP);
		if (vrs->map[(int)floor(new_y_test)][(int)floor(new_x_test)] == '0'
				&& vrs->map[(int)floor(new_y)][(int)floor(new_x)] == '0')
		{
			vrs->player_pos_x = new_x;
