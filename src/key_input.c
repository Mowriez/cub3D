...
/*   Updated: 2023/08/03 19:05:12 by mtrautne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	move_sideways(t_vars *vrs, int keycode)
{
	...
	double	new_y;
		if (keycode == KEY_A)
	{
		new_x = vrs->player_pos_x + cos(vrs->view_angle - 0.5 * M_PI) * moveSpeed;
		new_y = vrs->player_pos_y + sin(vrs->view_angle - 0.5 * M_PI) * moveSpeed;
		if (vrs->map[(int)floor(new_y)][(int)floor(new_x)] == '0')
		{
			vrs->player_pos_x = new_x;
			vrs->player_pos_y = new_y;
